...
class Game(object):
    ...
    def step(self, count=1):
        for generation in xrange(count):
            new_board = [[False] * self.width for row in xrange(self.height)]
            for y, row in enumerate(self.state.board):
                for x, cell in enumerate(row):
                    neighbours = self.neighbours(x, y)
                    previous_state = self.state.board[y][x]
                    should_live = neighbours == 3 or (
                        neighbours == 2 and previous_state == True)
        ...
        for hor in [-1, 0, 1]:
            for ver in [-1, 0, 1]:
                if not hor == ver == 0 and (self.infinite_board == True or (0 <= x + hor < self.width and 0 <= y + ver < self.height)):
                    count += self.state.board[(y + ver) %
                                              self.height][(x + hor) % self.width]
        return count
...
class State(object):
    ...
    def __init__(self, positions, x, y, width, height):
        ...
        board = [[False] * width for row in xrange(height)]
...
if __name__ == '__main__':
    ...
    my_game = Game(State(glider, x=2, y=3, width=40, height=30))
    for i in xrange(150):
        os.system('clear')
        my_game.step(1)
